...
$socialButtons = new Block('rrssb', [
    'url' => 'http://theurltoshare.com/example',
    'message' => 'Check out this cool website',
    'title' => 'Page Title',
    'image' => 'http://theurltoshare.com/share.jpg',
    'email' => true,
    'facebook' => true,
    'tumblr' => true,
    'linkedin' => true,
    'twitter' => true,
    'twitterMessage' => 'Check out this cool website #now @website',
    'reddit' => true,
    'googleplus' => true,
    'pinterest' => true,
    'pocket' => true,
...
$socialTypes = [
    ...
    'tumblr' =>     '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" width="28px" height="28px" viewBox="0 0 28 28" enable-background="new 0 0 28 28" xml:space="preserve"><path d="M18.02 21.842c-2.029 0.052-2.422-1.396-2.439-2.446v-7.294h4.729V7.874h-4.71V1.592c0 0-3.653 0-3.714 0 s-0.167 0.053-0.182 0.186c-0.218 1.935-1.144 5.33-4.988 6.688v3.637h2.927v7.677c0 2.8 1.7 6.7 7.3 6.6 c1.863-0.03 3.934-0.795 4.392-1.453l-1.22-3.539C19.595 21.6 18.7 21.8 18 21.842z"/></svg>',
    ...
    'youtube' =>    '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="28px" height="28px" viewBox="0 0 28 28" enable-background="new 0 0 28 28" xml:space="preserve"><path d="M27.688,8.512c0-2.268-1.825-4.093-4.106-4.093H4.389c-2.245,0-4.076,1.825-4.076,4.093v10.976 c0,2.268,1.825,4.093,4.076,4.093h19.192c2.274,0,4.106-1.825,4.106-4.093V8.512z M11.263,18.632V8.321l7.817,5.155L11.263,18.632z"/></svg>',
    ...
    'pocket' =>     '<svg width="32px" height="28px" viewBox="0 0 32 28" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns"><path d="M28.7817528,0.00172488695 C30.8117487,0.00431221738 31.9749312,1.12074529 31.9644402,3.10781507 C31.942147,6.67703739 32.1336065,10.2669583 31.8057648,13.8090137 C30.7147076,25.5813672 17.2181194,31.8996281 7.20714461,25.3808491 C2.71833574,22.4571656 0.196577202,18.3122624 0.0549495772,12.9357897 C-0.0342233715,9.5774348 0.00642900214,6.21519891 0.0300336062,2.85555035 C0.0405245414,1.1129833 1.21157517,0.0146615391 3.01995012,0.00819321302 C7.34746087,-0.00603710433 11.6775944,0.00431221738 16.0064164,0.00172488695 C20.2644248,0.00172488695 24.5237444,-0.00215610869 28.7817528,0.00172488695 L28.7817528,0.00172488695 Z M8.64885184,7.85611511 C7.38773662,7.99113854 6.66148108,8.42606978 6.29310958,9.33228474 C5.90114134,10.2969233 6.17774769,11.1421181 6.89875951,11.8276216 C9.35282156,14.161969 11.8108164,16.4924215 14.2976518,18.7943114 C15.3844131,19.7966007 16.5354102,19.7836177 17.6116843,18.7813283 C20.0185529,16.5495467 22.4070683,14.2982907 24.7824746,12.0327533 C25.9845979,10.8850542 26.1012707,9.56468083 25.1469132,8.60653379 C24.1361858,7.59255976 22.8449191,7.6743528 21.5890476,8.85191291 C19.9936451,10.3488554 18.3680912,11.8172352 16.8395462,13.3777945 C16.1342655,14.093159 15.7200114,14.0048744 15.0566806,13.3440386 C13.4599671,11.7484252 11.8081945,10.2060421 10.1262706,8.70001155 C9.65564653,8.27936164 9.00411403,8.05345704 8.64885184,7.85611511 L8.64885184,7.85611511 L8.64885184,7.85611511 Z"></path></svg>',
...
$rrssbGetValue = function($block, $type, $key) {
  return isset($block->model["{$type}{$key}"]) ?
    $block->model["{$type}{$key}"] :
    $block->model[strtolower($key)];
};

foreach ($socialTypes as $socialType => $icon) {
    if (isset($block->model[$socialType]) && $block->model[$socialType]) {
      $href = '';
      $popup = false;
      $message = $rrssbGetValue($block, $socialType, 'Description');
      $title = $rrssbGetValue($block, $socialType, 'Name');
      $url = $rrssbGetValue($block, $socialType, 'Url');
      $image = $rrssbGetValue($block, $socialType, 'Image');
      $text = $socialType;
      switch($socialType) {
          case 'email':
            $href = sprintf(
          ...
          case 'tumblr':
            $href = sprintf(
                'http://tumblr.com/share?s=&amp;v=3&t=%s&amp;u=%s',
                urlencode($title),
                urlencode($url)
            );
            break;
          ...
          case 'twitter':
            $href = sprintf(
                'http://twitter.com/home?status=%s%s',
                urlencode($message . ' '),
                $url
            );
            $popup = true;
          ...
          case 'googleplus':
            $href = sprintf(
                'https://plus.google.com/share?url=%s%s',
                urlencode($message . ' '),
                $url
            );
            $text = 'google+';
...
?>
<li class="rrssb-<?php echo $socialType; ?>">
    <a href="<?php echo $href; ?>">
        <span class="rrssb-icon">
          <?php echo $icon; ?>
        </span>
        <span class="rrssb-text"><?php echo $text; ?></span>
    </a>
</li>
<?php
    }
